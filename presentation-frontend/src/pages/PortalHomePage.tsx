    ...
    <div style={{ maxWidth: 1200, fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 32 }}>
        <QuickCard
          ...
          iconBg='#e3f2fd'
          title='Programação'
          desc='Veja os filmes em cartaz hoje'
        ...
        <QuickCard
          ...
          desc='Reserve seu assento e finalize a compra'
        ...
        <QuickCard
          ...
          iconBg='#fce4ec'
          title='Fidelidade'
          desc='Consulte seus pontos e benefícios'
          onClick={() => navigate('/fidelidade')}
        />
      </div>

      {/* Seção principal: banner + próximas sessões */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 20, marginBottom: 32 }}>
        ...
        <div style={{
          ...
          minHeight: 300,
          position: 'relative',
          background: BANNER_GRADIENTS[destaque % BANNER_GRADIENTS.length],
          ...
          {loading ? (
            ...
            <div style={{ padding: 36, display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box', minHeight: 300 }}>
              ...
              <div style={{ fontSize: 26, fontWeight: 800, color: 'white', marginBottom: 12, lineHeight: 1.2 }}>
              ...
              {filmeDestaque.sinopse && (
                <p style={{ margin: '0 0 20px', color: 'rgba(255,255,255,0.85)', fontSize: 14, lineHeight: 1.6, maxWidth: 420 }}>
                  {filmeDestaque.sinopse.length > 140
                    ? filmeDestaque.sinopse.slice(0, 140) + '…'
                    : filmeDestaque.sinopse}
                </p>
              )}
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 24 }}>
                ...
                <span style={badgeStyle('rgba(255,255,255,0.2)')}>
                  {filmeDestaque.duracaoMinutos} min
                </span>
                {filmeDestaque.nota > 0 && (
                  <span style={badgeStyle('rgba(255,200,0,0.3)')}>★ {filmeDestaque.nota.toFixed(1)}</span>
                )}
                <span style={badgeStyle('rgba(255,255,255,0.2)')}>
                  {CLASSIFICACAO_LABEL[filmeDestaque.classificacao] ?? filmeDestaque.classificacao}
                </span>
              </div>
              <button
                onClick={() => navigate('/programacao')}
                style={{
                  ...
                  background: 'white', color: COR, border: 'none',
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 'auto', paddingTop: 20 }}>
                  {filmes.slice(0, 3).map((_, i) => (
                    <button
                      ...
                      style={{
                        width: i === destaque ? 20 : 8, height: 8,
            ...
            <div style={{ padding: 36, display: 'flex', flexDirection: 'column', height: 300, justifyContent: 'center' }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 16, fontWeight: 600 }}>
                Nenhum filme em cartaz hoje
              </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
            ...
            <button
              onClick={() => navigate('/programacao')}
              style={{ background: 'none', border: 'none', color: COR, fontSize: 13, cursor: 'pointer', fontWeight: 500 }}>
              Ver todas
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ color: '#bbb', fontSize: 14, textAlign: 'center', padding: '20px 0' }}>
              Nenhuma sessão hoje
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {proximasSessoes.map((s, i) => (
                <div key={s.id} style={{
                  ...
                  paddingBottom: i < proximasSessoes.length - 1 ? 12 : 0,
                  borderBottom: i < proximasSessoes.length - 1 ? '1px solid #f5f5f5' : 'none',
                }}>
                  <div style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: '#999', marginTop: 2 }}>
                      Hoje • {s.inicio.slice(0, 5)} • Sala {String(s.sala).padStart(2, '0')}
                    </div>
                  </div>
...
function badgeStyle(bg: string): React.CSSProperties {
  return {
    background: bg, color: 'white', fontSize: 12,
